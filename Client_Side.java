...
public class Client_Side extends JFrame {
    ...
    private void initComponents() {
        ...
        serverAddressField = new JTextField("10.100.32.201", 10);  // default IP
        ...
        // Disconnect button panel
        gbc.gridx = 0; gbc.gridy = 0;
        jPanel4.add(new JLabel("Disconnect:"), gbc);
        gbc.gridx = 1; gbc.gridy = 0;
        jPanel4.add(disconnectButton, gbc);

        // Message input + send button panel
        gbc.gridx = 0; gbc.gridy = 0;
        jPanel2.add(new JLabel("Enter Message:"), gbc);
        gbc.gridx = 0; gbc.gridy = 1;
    ...
    private void connect() {
        ...
        username = clientNameField.getText().trim();
        address = serverAddressField.getText().trim();
        ...
        try {
            socket = new Socket(address, 5000);  // Use port 5000 as your server uses
            ...
            messageArea.append("Connected to server at " + address + ":5000\n");
            listenForMessages();
        } catch (Exception ex) {
    ...
    private void sendDisconnect() {
        try {
            if (printWriter != null) {
                printWriter.println(username + ":Disconnect");
                printWriter.flush();
            }
        } catch (Exception e) {
    ...
    private void sendMessage() {
        ...
        try {
            printWriter.println(username + ":" + msg + ":Chat");
            printWriter.flush();
        } catch (Exception e) {
                ...
                while ((stream = bufferedReader.readLine()) != null) {
                    ...
                    if (data.length >= 3) {
                        ...
                        if (type.equals("Chat")) {
                            ...
                            // Optionally handle new user connected
                            messageArea.append(data[0] + " connected.\n");
                        } else if (type.equals("Disconnect")) {
                            // Optionally handle user disconnected
                            messageArea.append(data[0] + " disconnected.\n");
                        }
                    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new Client_Side().setVisible(true));
    }
}
